...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function saveCurrentUiState() {
        const stateToSave = {
            ...
            last_language: languageSelect ? languageSelect.value : currentUiState.last_language || 'en',
    ...
    function loadInitialUiState() {
        ...
        if (languageSelect) {
            const fallbackLanguage = genDefaults.language || 'en';
            const preferredLanguage = currentUiState.last_language || fallbackLanguage;
            const hasPreferred = Array.from(languageSelect.options).some(opt => opt.value === preferredLanguage);
            languageSelect.value = hasPreferred
                ? preferredLanguage
                : (languageSelect.options[0]?.value || fallbackLanguage);
        }
