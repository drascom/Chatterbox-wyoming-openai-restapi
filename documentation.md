...
|                       | `multilingual_model_enabled`  | boolean       | Whether to load `ChatterboxMultilingualTTS` for multilingual generation when non-English languages are requested. | `false`                  |
|                       | `voice_language_map`          | object        | Map of voice filename to language code for HA/Wyoming display and language inference (e.g., `{"Abigail.wav":"en"}`). | `{}`                     |
...
|                       | `last_language`               | string        | Last language selected in the UI.                                                                             | `en`                     |
...
|                       | `languages`                   | list          | Language options displayed in the dropdown. Each entry is an object with `code` and `label`.                   | `[{"code":"en","label":"English"},{"code":"tr","label":"Türkçe"}]` |
