...
async def custom_tts_endpoint(
    ...
    language_to_use = request.language or get_gen_default_language()
    logger.info(
        f"Received /tts request: mode='{request.voice_mode}', format='{request.output_format}', language='{language_to_use}'"
    )
    logger.debug(
